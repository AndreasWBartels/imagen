 ...
 *
 * <p>
 * The PNM file format includes PBM for monochrome images, PGM for grey scale images, and PPM for color images. When
 ...
 *
 * <p>
 * The raw file format is used wherever possible, unless the PNMEncodeParam object supplied to the constructor returns
...
public class PNMImageEncoder extends ImageEncoderImpl {
  ...
  public PNMImageEncoder(final OutputStream output,
  ...
  public void encode(final RenderedImage im) throws IOException {
    ...
    // Write PNM file.
    this.output.write('P'); // magic value
    this.output.write(this.variant);

    this.output.write(this.lineSeparator);
    this.output.write(COMMENT.getBytes()); // comment line

    this.output.write(this.lineSeparator);
    writeInteger(this.output, width); // width
    this.output.write(SPACE);
    writeInteger(this.output, height); // height
    ...
    } else if ((this.variant == PGM_RAW || this.variant == PPM_RAW) &&
    ...
    if (writeOptimal) {
      int bytesPerRow = this.variant == PBM_RAW ? (width + 7) / 8 : width * sampleModel.getNumBands();
      ...
      for (int j = 0; j < numYTiles; j++) {
        ...
        int rowStride =
            this.variant == PBM_RAW ? ((MultiPixelPackedSampleModel) strip.getSampleModel()).getScanlineStride()
        ...
        case PPM_ASCII:
          if (reds == null) { // no need to expand
            ...
            for (int i = 0; i < size; i++) {
              ...
              writeInteger(this.output, (reds[pixels[i]] & 0xFF));
              this.output.write(SPACE);
              writeInteger(this.output, (greens[pixels[i]] & 0xFF));
              this.output.write(SPACE);
              writeInteger(this.output, (blues[pixels[i]] & 0xFF));
            }
          }
          this.output.write(this.lineSeparator);
          break;
  ...
  private void writeInteger(final OutputStream output, final int i) throws IOException {
    output.write(Integer.toString(i).getBytes());
  }

  /** Writes a byte to the output in ASCII format. */
  private void writeByte(final OutputStream output, final byte b) throws IOException {
