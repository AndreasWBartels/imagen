...
/**
 * A baseline TIFF writer. The writer outputs TIFF images in either Bilevel, Greyscale, Palette color or Full Color
 * modes.
 *
 * @since EA4
 */
public class TIFFImageEncoder extends ImageEncoderImpl {
  ...
  private static final char[] intsToChars(final int[] intArray) {
  ...
  private int encode(RenderedImage im,
      final TIFFEncodeParam encodeParam,
      final int ifdOffset,
    ...
    if (colorModel instanceof IndexColorModel) { // Bilevel or palette
      ...
      if (numExtraSamples == 1 && colorModel.hasAlpha()) {
        extraSampleType =
            colorModel.isAlphaPremultiplied() ? EXTRA_SAMPLE_ASSOCIATED_ALPHA : EXTRA_SAMPLE_UNASSOCIATED_ALPHA;
      }
    }
...
//            }
    ...
    if ((imageType == TIFF_BILEVEL_BLACK_IS_ZERO ||
      ...
      fields.add(new TIFFField(TIFFImageDecoder.TIFF_FILL_ORDER,
          TIFFField.TIFF_SHORT,
          1,
          new char[] { inverseFill ? (char) 2 : (char) 1 }));
    ...
    if (imageType == TIFF_YCBCR) {
      ...
      fields.add(new TIFFField(TIFF_YCBCR_POSITIONING,
          TIFFField.TIFF_SHORT,
          1,
          new char[] { compression == COMP_JPEG_TTN2 ? (char) 1 : (char) 2 }));
    ...
    for (int row = minY; row < lastRow; row += tileHeight) {
      ...
      for (int col = minX; col < lastCol; col += tileWidth) {
        ...
        if (compression != COMP_JPEG_TTN2) { // JPEG access Raster
          ...
          if (!useDataBuffer) {
            if (dataType == DataBuffer.TYPE_FLOAT) {
              fpixels = src.getPixels(col,
                  row,
                  tileWidth,
                  rows,
                  fpixels);
            } else {
              pixels = src.getPixels(col,
                  row,
                  tileWidth,
                  rows,
                  pixels);
              ...
              for (int tileRow = 0; tileRow < rows; tileRow++) {
                int numCompressedBytesInRow =
                    faxEncoder.encodeRLE(bpixels,
                        ...
                        tileWidth,
                        compressBuf);
                this.output.write(compressBuf,
                    0,
                    numCompressedBytesInRow);
                rowOffset += rowStride;
   ...
   */
  private int getDirectorySize(final SortedSet fields) {
  ...
  private void writeDirectory(final int thisIFDOffset,
      final SortedSet fields,
   ...
   */
  private static final int getValueSize(final TIFFField field) {
    int type = field.getType();
    int count = field.getCount();
    int valueSize = 0;
  ...
  private void writeValuesAsFourBytes(final TIFFField field) throws IOException {
  ...
  private void writeValues(final TIFFField field) throws IOException {
  ...
  // stored in 2 bytes.
  private void writeUnsignedShort(final int s) throws IOException {
    if (this.isLittleEndian) {
      this.output.write(s & 0x00ff);
  ...
  private void writeLong(final long l) throws IOException {
    if (this.isLittleEndian) {
      this.output.write(((int) l & 0x000000ff));
      this.output.write((int) ((l & 0x0000ff00) >>> 8));
      this.output.write((int) ((l & 0x00ff0000) >>> 16));
      this.output.write((int) ((l & 0xff000000) >>> 24));
    } else {
      this.output.write((int) ((l & 0xff000000) >>> 24));
      this.output.write((int) ((l & 0x00ff0000) >>> 16));
      this.output.write((int) ((l & 0x0000ff00) >>> 8));
  ...
  /**
   * Returns the current offset in the supplied OutputStream. This method should only be used if compressing data.
   */
  private long getOffset(final OutputStream out) throws IOException {
   ...
   */
  private static int compressPackBits(final byte[] data,
      final int numRows,
  ...
  /**
   * Performs PackBits compression for a single buffer of data. This should be called for each row of each tile. The
   * returned value is the offset into the output buffer after compression.
   */
  private static int packBits(final byte[] input,
      int inOffset,
      final int inCount,
      final byte[] output,
      int outOffset) {
  ...
  private static int deflate(final Deflater deflater,
      final byte[] inflated,
      final byte[] deflated) {
