...
public class TIFFImageOrginal extends SimpleRenderedImage {
  ...
  /**
   * Inflates <code>deflated</code> into <code>inflated</code> using the <code>Inflater</code> constructed during class
   * instantiation.
   */
...
//                                       dfe.getMessage());
  ...
  /**
   * Creates a pixel-interleaved <code>SampleModel</code>. This is a hack to work around a cast exception when using JAI
   * with float data.
   */
  private final static SampleModel
      createPixelInterleavedSampleModel(final int dataType,
          final int tileWidth,
    ...
    if (dataType == DataBuffer.TYPE_FLOAT) {
      ...
      try {
        Class rfClass =
            Class.forName("org.eclipse.imagen.RasterFactory");
  ...
  /*
   * Check whether the specified tag exists in the specified TIFFDirectory. If not, throw an error message. Otherwise
  ...
  /**
   * Constructs a TIFFImage that acquires its data from a given SeekableStream and reads from a particular IFD of the
   * stream. The index of the first IFD is 0.
   *
   ...
   */
  public TIFFImageOrginal(final SeekableStream stream,
    ...
    TIFFDirectory dir = param.getIFDOffset() == null ? new TIFFDirectory(stream, directory)
        : new TIFFDirectory(stream,
            param.getIFDOffset().longValue(),
            directory);
    ...
    switch (this.sampleSize) {
      ...
      case 32:
        ...
        break;
      case 64:
        this.dataType = sampleFormat[0] == 3 ? DataBuffer.TYPE_DOUBLE : DataBuffer.TYPE_INT;
        isValidDataFormat = true;
        break;
    }
    ...
    if (dir.getField(TIFFImageDecoder.TIFF_TILE_OFFSETS) != null) {
      ...
      this.tileWidth = (int) (getField(dir,
          TIFFImageDecoder.TIFF_TILE_WIDTH,
          "Tile Width").getAsLong(0));
      this.tileHeight = (int) (getField(dir,
      ...
      this.tileByteCounts = getFieldAsLongs(
          getField(dir,
      ...
      this.tileWidth =
          dir.getField(TIFFImageDecoder.TIFF_TILE_WIDTH) != null
              ? (int) dir.getFieldAsLong(TIFFImageDecoder.TIFF_TILE_WIDTH) : this.width;
      TIFFField field =
      ...
      if (field == null) {
        ...
        this.tileHeight =
            dir.getField(TIFFImageDecoder.TIFF_TILE_LENGTH) != null
                ? (int) dir.getFieldAsLong(TIFFImageDecoder.TIFF_TILE_LENGTH) : this.height;
      } else {
      ...
      if (this.tileByteCounts.length == 1 &&
          this.compression == COMP_NONE &&
    ...
    switch (this.compression) {
      ...
      case COMP_FAX_G4_2D:
        ...
        this.decoder = new TIFFFaxDecoder(this.fillOrder,
            this.tileWidth,
            this.tileHeight);
        break;
      ...
      case COMP_LZW:
        ...
        if (predictorField == null) {
          ...
          if (this.predictor == 2 && this.sampleSize != 8) {
            throw new RuntimeException(this.sampleSize +
                JaiI18N.getString("TIFFImage9"));
          }
        }

        this.lzwDecoder = new TIFFLZWDecoder(this.tileWidth,
            this.predictor,
            samplesPerPixel);
        break;
    ...
    switch (this.imageType) {
      ...
      case TYPE_GRAY_4BIT:
        this.sampleModel =
            new MultiPixelPackedSampleModel(this.dataType,
                this.tileWidth,
                this.tileHeight,
                this.sampleSize);
        if (this.imageType == TYPE_BILEVEL) {
          byte[] map = new byte[] { (byte) (this.isWhiteZero ? 255 : 0),
              (byte) (this.isWhiteZero ? 0 : 255) };
          this.colorModel = new IndexColorModel(1, 2, map, map, map);
        } else {
          this.colorModel =
              ImageCodec.createGrayIndexColorModel(this.sampleModel,
                  !this.isWhiteZero);
        }
        break;
      ...
      case TYPE_CMYK:
        ...
        this.sampleModel = createPixelInterleavedSampleModel(this.dataType,
            this.tileWidth,
            this.tileHeight,
            this.numBands,
        ...
        if (this.imageType == TYPE_GRAY || this.imageType == TYPE_RGB) {
          this.colorModel =
              ImageCodec.createComponentColorModel(this.sampleModel);
          ...
          this.colorModel =
              createAlphaComponentColorModel(this.dataType,
                  this.numBands,
                  extraSamples == 1,
                  transparency);
        }
      ...
      case TYPE_YCBCR_SUB:
        ...
        this.sampleModel =
            createPixelInterleavedSampleModel(this.dataType,
                this.tileWidth,
                this.tileHeight,
                this.numBands,
                this.numBands * this.tileWidth,
      ...
      case TYPE_PALETTE:
        // Get the colormap
        TIFFField cfield = getField(dir,
            TIFFImageDecoder.TIFF_COLORMAP,
            "Colormap");
        ...
        if (this.decodePaletteAsShorts) {
          ...
          this.sampleModel =
              RasterFactory.createPixelInterleavedSampleModel(this.dataType,
                  this.tileWidth,
                  this.tileHeight,
                  this.numBands);
          this.colorModel = ImageCodec.createComponentColorModel(this.sampleModel);
          ...
          if (this.dataType == DataBuffer.TYPE_SHORT) {
            ...
            for (int i = 0; i < bandLength; i++) {
              r[i] = param.decodeSigned16BitsTo8Bits(
                  (short) this.colormap[i]);
              g[i] = param.decodeSigned16BitsTo8Bits(
                  (short) this.colormap[gIndex + i]);
              b[i] = param.decodeSigned16BitsTo8Bits(
                  (short) this.colormap[bIndex + i]);
            }
          ...
          this.colorModel = new IndexColorModel(this.sampleSize,
              bandLength,
              r,
              g,
              b);
  ...
  /**
   * Reads a private IFD from a given offset in the stream. This method may be used to obtain IFDs that are referenced
  ...
  public synchronized Raster getTile(final int tileX, final int tileY) {
    // Check parameters.
    if ((tileX < 0) || (tileX >= this.tilesX) ||
        (tileY < 0) || (tileY >= this.tilesY)) {
...
//                    throw new RuntimeException(JaiI18N.getString("TIFFImage18"));
      ...
      tile =
          RasterFactory.createWritableRaster(this.sampleModel,
              buffer,
...
//	    throw new RuntimeException(JaiI18N.getString("TIFFImage13"));
      ...
      Rectangle tileRect = new Rectangle(tileXToX(tileX),
          tileYToY(tileY),
          this.tileWidth,
          this.tileHeight);
      Rectangle newRect = this.isTiled ? tileRect : tileRect.intersection(getBounds());
...
//					JaiI18N.getString("TIFFImage13"));
            ...
            if (dataType == DataBuffer.TYPE_USHORT) {
              ...
              // data type.
              int cmapValue;
              int count = 0, lookup, len = this.colormap.length / 3;
              int len2 = len * 2;
        ...
        try {
          ...
          if (this.sampleSize == 8) {
            ...
            if (this.compression == COMP_NONE) {
              ...
              this.lzwDecoder.decode(data, byteArray, newRect.height);
              interpretBytesAsInts(byteArray,
                  idata,
                  unitsInThisTile);
            ...
            if (this.compression == COMP_NONE) {
              ...
              byte byteArray[] = new byte[unitsInThisTile * 4];
              this.lzwDecoder.decode(data, byteArray, newRect.height);
              interpretBytesAsFloats(byteArray,
                  fdata,
                  unitsInThisTile);
        ...
        switch (this.imageType) {
          ...
          case TYPE_GRAY_ALPHA:
            if (this.isWhiteZero) {
              ...
              if (dataType == DataBuffer.TYPE_BYTE &&
                  !(this.colorModel instanceof IndexColorModel)) {
...
//	   throw new RuntimeException(JaiI18N.getString("TIFFImage13"));
  ...
  // whether the bytes are stored in a big endian or little endian format.
  private void interpretBytesAsFloats(final byte byteArray[],
...
/**
 * Wrapper class for a <code>SeekableStream</code> but which does not throw an <code>EOFException</code> from
 * <code>readFully()</code> when the end of stream is encountered.
 */
