 ...
 *
 * <p>
 * Sample values are taken as ranging from 0.0 to 1.0; that is, when converting to 8-bit RGB, a multiplication by 255 is
 * performed and values outside of the range 0-255 are clamped at the closest endpoint.
 *
 * <p>
 * For maximum efficiency, pixel data being interpreted by this class should be in the sRGB color space. This will
 ...
 *
 * <p>
 * For those methods that use a primitive array pixel representation of type <code>transferType</code>, the array length
 * is the same as the number of color and alpha samples. Color samples are stored first in the array followed by the
 * alpha sample, if present. The order of the color samples is specified by the <code>ColorSpace</code>. Typically, this
 * order reflects the name of the color space type. For example, for <code>TYPE_RGB</code>, index 0 corresponds to red,
 * index 1 to green, and index 2 to blue. The transfer types supported are <code>DataBuffer.TYPE_FLOAT</code>,
 * <code>DataBuffer.TYPE_DOUBLE</code>.
 *
 * <p>
 * The translation from pixel values to color/alpha components for display or processing purposes is a one-to-one
 ...
 *
 * <p>
 * A <code>FloatDoubleColorModel</code> can be used in conjunction with a <code>ComponentSampleModelJAI</code>.
 *
...
public class FloatDoubleColorModel extends ComponentColorModel {
   ...
   * <code>hasAlpha</code> is true, then the boolean <code>isAlphaPremultiplied</code> specifies how to interpret color
   * and alpha samples in pixel values. If the boolean is <code>true</code>, color samples are assumed to have been
   * multiplied by the alpha sample. The <code>transparency</code> specifies what alpha values can be represented by
   * this color model. The <code>transferType</code> is the type of primitive array used to represent pixel values.
   *
  ...
  /**
   * Returns the alpha component for the specified pixel, scaled from 0 to 255. The pixel value is specified by an array
   * of data elements of type <code>transferType</code> passed in as an object reference. If the <code>ColorModel</code>
   * does not have alpha, 255 is returned.
   *
   ...
   *
   * @throws IllegalArgumentException       if <code>inData</code> is <code>null</code> and the <code>colorModel</code>
   *                                        has alpha.
   * @throws ClassCastException             If <code>inData</code> is not a primitive array of type
   *                                        <code>transferType</code> and the <code>ColorModel</code> has alpha.
   * @throws ArrayIndexOutOfBoundsException if <code>inData</code> is not large enough to hold a pixel value for this
  ...
  public Object getDataElements(final int rgb, final Object pixel) {
    if (this.transferType == DataBuffer.TYPE_FLOAT) {
      ...
      if (this.colorSpace.isCS_sRGB()) {
        ...
        if (this.hasAlpha) {
          int alpha = (rgb >> 24) & 0xff;
          floatPixel[this.numColorComponents] = alpha * inv255;
        }
      }
      ...
      if (this.colorSpace.isCS_sRGB()) {
        ...
        if (this.hasAlpha) {
          int alpha = (rgb >> 24) & 0xff;
          doublePixel[this.numColorComponents] = alpha * inv255;
        }
      }
  ...
  public Object getDataElements(final int[] components, final int offset, final Object obj) {
    if ((components.length - offset) < this.numComponents) {
      throw new IllegalArgumentException(this.numComponents + " " +
  ...
  /**
   * Forces the <code>raster</code> data to match the state specified in the <code>isAlphaPremultiplied</code> variable,
   * assuming the data is currently correctly described by this <code>ColorModel</code>. It may multiply or divide the
   * color <code>raster</code> data by alpha, or do nothing if the data is in the correct state. If the data needs to be
   * coerced, this method also returns an instance of <code>FloatDoubleColorModel</code> with the
   * <code>isAlphaPremultiplied</code> flag set appropriately.
   *
   * @throws IllegalArgumentException if transfer type of <code>raster</code> is not the same as that of this
  ...
  public ColorModel coerceData(final WritableRaster raster,
      ...
      switch (this.transferType) {
        case DataBuffer.TYPE_FLOAT: {
          ...
          for (int y = 0; y < h; y++, rY++) {
            ...
            for (int x = 0; x < w; x++, rX++) {
              pixel = (float[]) raster.getDataElements(rX,
                  rY,
                  pixel);
        ...
        case DataBuffer.TYPE_DOUBLE: {
          ...
          for (int y = 0; y < h; y++, rY++) {
            rX = rminX;
            for (int x = 0; x < w; x++, rX++) {
              pixel = (double[]) raster.getDataElements(rX,
                  rY,
                  pixel);
    ...
    return new FloatDoubleColorModel(this.colorSpace,
        this.hasAlpha,
        isAlphaPremultiplied,
        this.transparency,
        this.transferType);
  ...
  /**
   * Creates a <code>WritableRaster</code> with the specified width and height, that has a data layout
   * (<code>SampleModel</code>) compatible with this <code>ColorModel</code>. The returned <code>WritableRaster</code>'s
  ...
  /**
   * Creates a <code>SampleModel</code> with the specified width and height that has a data layout compatible with this
   * <code>ColorModel</code>. The returned <code>SampleModel</code> will be an instance of
  ...
  public SampleModel createCompatibleSampleModel(final int w, final int h) {
    ...
    return new ComponentSampleModelJAI(this.transferType,
        w,
        h,
        this.numComponents,
        w * this.numComponents,
        bandOffsets);
  }
