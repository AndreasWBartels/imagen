 ...
 *
 * When the statistic parameters are displayed, the following parameters of
 *  a rectangular ROI are listed in a table: the area, the width, the height,
 *  the maximum, the minimum, the mean, the standard deviation and the entropy
 *  of the values of the pixels located in this ROI.
 *
 */


public class HistogramFrame extends JFrame {
    /** The LUT (look-up-table) to map the 12-bit image into 8-bit image.  Used
     *  to define the color of the bar for each bin of the histogram when
     *  display.
     */
    ...
    /**
     * The ROI on which the histogram or the statistics is calculated.  It is
     *  defined in the physical coordinate system.
     */
     ...
     *  @param lut The lut for window/level operation. Used to define the color
     *		   of each bin of the histogram when display.
     *  @param roi The ROI in the original image coordinate system.
     */


    public HistogramFrame(Histogram h,
                          boolean displayHistogram,
			  byte[][] lut,
                          Shape roi) {
	this.lut = lut;
	this.histogram = h;
	...
	if (displayHistogram) {
	    ...
	    ImagePane pane =
		new ImagePane(displayHistogram(h), "Histogram");
     ...
     *  parameters are calculated and displayed: the area, the width, the
     *  height, the maximum, the minimum, the mean, the standard deviation
     *  and the entropy of the values of the pixels located in the ROI.
     */
	...
	String[][] content = new String[][]{{"Area (mm2)", "" + getArea(roi)},
	...
	for(int b = 0; b < numBands; b++) {
		...
		if (bins[i] > 0) {
		    minValue[b] = (int)(i * (highValue[b] - lowValue[b]) / numBins +
				  lowValue[b]);
		    break;
		}
	    }
    ...
    private int[] getMaxValue() {
        ...
        for(int b = 0; b < numBands; b++) {
            ...
            for(int i = numBins - 1; i >= 0; i--) {
                if (bins[i] > 0) {
                    maxValue[b] = (int)(i * (highValue[b] - lowValue[b]) / numBins +
                                  lowValue[b]);
                    break;
		}
            }
    ...
    /**
     * Draw the histogram in a RenderedImage for display.  The color of the
     *  bar for each bin is defined by the provided LUT.
     *
    ...
    private RenderedImage displayHistogram(Histogram h) {
        ...
        for (int b = 1; b < numBands; b++) {
            if (h.getNumBins(b) != numBins) {
                throw
                    new RuntimeException("All bands must have same numBins.");
            }
        }
